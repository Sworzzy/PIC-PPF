...
def cyclotron_xy(x0, v0, q, m, Bz, times):
    ...
    for i in range(len(times)) :
        ...
        v_analytic[i, 0] = v0[0] * np.cos(omega * times[i]) + v0[1] * np.sin(omega * times[i])
        v_analytic[i, 1] = v0[1] * np.cos(omega * times[i]) - v0[0] * np.sin(omega * times[i])
...
# -------------------------------------------------
def ExB_xy(x0, v0, q, m, E, B, times):
    """
    ...
    x_cyc, v_cyc = cyclotron_xy(x0, v_rel0, q, m, Bz, times)
